...
const modbus_wallbox_bridge_1 = require("./modbus-wallbox-bridge");
let wallboxManager = null;
function getWallboxManager() {
    if (!wallboxManager) {
        ...
        wallboxManager = new modbus_wallbox_bridge_1.WallboxBridgeManager(cloudWsBase, password);
    }
    return wallboxManager;
}
async function fetchWallboxInstanceAndTemplate(instanceId) {
    try {
        ...
        console.error("[wb-bridge] fetch instance failed", e.message);
        return null;
    }
}
...
function loadConfig() {
    ...
    try {
        const raw = fs_1.default.readFileSync(optionsPath, "utf-8");
...
function applyRemoteConfig(incoming, version) {
...
async function cloudAuthHeaders(extra = {}) {
    const headers = {
...
let currentAssignmentStatus = "unknown";
/* ── PIN Auth State ──────────────────────────────────────────────────────────── */
const crypto_1 = __importDefault(require("crypto"));
let uiPinHash = null; // SHA-256 hash synced from cloud
...
`);
function loadRemoteConfigFromCache() {
    try {
        const row = db
...
function saveRemoteConfigToCache(cfg, version) {
    try {
        db.prepare(`INSERT INTO gateway_remote_config (cache_key, config_json, version, updated_at)
       VALUES ('primary', ?, ?, datetime('now'))
       ON CONFLICT(cache_key) DO UPDATE SET
         config_json = excluded.config_json,
         version = excluded.version,
         updated_at = excluded.updated_at`).run(JSON.stringify(cfg), version);
    }
    catch (err) {
...
const insertReading = db.prepare(`INSERT INTO readings_buffer (meter_id, tenant_id, power_value, energy_type, recorded_at, priority) VALUES (?, ?, ?, ?, ?, ?)`);
const fetchBatch = db.prepare(`SELECT id, meter_id, tenant_id, power_value, energy_type, recorded_at FROM readings_buffer ORDER BY id LIMIT ?`);
...
function getPriority(powerValue, energyType) {
...
function enforceBufferLimit() {
    ...
    try {
        ...
        if (stats.size > maxBytes) {
            ...
            const oldest = db.prepare(`SELECT id FROM readings_buffer WHERE priority = 0 ORDER BY id LIMIT ?`).all(toDelete);
            if (oldest.length > 0) {
                deleteBatch.run(oldest[oldest.length - 1].id);
                console.log(`[buffer] Evicted ${oldest.length} non-priority readings (FIFO)`);
            }
            else {
                const anyOldest = db.prepare(`SELECT id FROM readings_buffer ORDER BY id LIMIT ?`).all(toDelete);
...
function matchesEntityFilter(entityId) {
    if (entityFilterPatterns.length === 0)
        return true;
    return entityFilterPatterns.some((re) => re.test(entityId));
}
// ── NEW: Persist meter mappings to SQLite cache ──
function saveMeterMappingsToCache(mappings) {
    const upsert = db.prepare(`INSERT OR REPLACE INTO meter_mappings_cache (id, sensor_uuid, energy_type, tenant_id) VALUES (?, ?, ?, ?)`);
...
function loadMeterMappingsFromCache() {
    return db.prepare(`SELECT id, sensor_uuid, energy_type, tenant_id FROM meter_mappings_cache`).all();
}
async function fetchMeterMappings() {
    if (!isCloudReachable) {
    ...
    try {
        ...
        if (data.success && Array.isArray(data.meters)) {
            meterMappings = data.meters
                .filter((m) => m.sensor_uuid && m.capture_type === "automatic")
                .map((m) => ({
                id: m.id,
...
function saveHAStatesToCache() {
    const upsert = db.prepare(`INSERT OR REPLACE INTO ha_states_cache (entity_id, state, attributes, last_updated) VALUES (?, ?, ?, ?)`);
...
function loadHAStatesFromCache() {
    const rows = db.prepare(`SELECT entity_id, state, attributes, last_updated FROM ha_states_cache`).all();
    return rows.map(r => ({
...
async function pollHAStates() {
    try {
        ...
        for (const state of states) {
            if (!matchesEntityFilter(state.entity_id))
                continue;
            ...
            insertReading.run(mapping.id, mapping.tenant_id, value, mapping.energy_type, state.last_updated || new Date().toISOString(), priority);
            buffered++;
        }
        if (buffered > 0) {
...
function getLocalAutomations() {
    return db.prepare(`SELECT * FROM automations_local`).all();
}
function updateLocalExecutionTime(id) {
...
function isTimeInRange(currentTime, timeFrom, timeTo) {
    if (timeFrom <= timeTo) {
...
function isExactTimePoint(totalSeconds, targetTime, toleranceSec = 30) {
...
async function evaluateAndExecuteAutomations() {
    ...
    for (const auto of automations) {
        ...
        if (Date.now() - lastExec < DEBOUNCE_MS)
            continue;
        ...
        for (const cond of conditions) {
            ...
            switch (cond.type) {
                ...
                case "time_point":
                    if (cond.time_point)
                        result = isExactTimePoint(timeParts.totalSeconds, cond.time_point);
                    break;
                case "time_switch":
                    if (cond.time_points?.length > 0) {
                        result = cond.time_points.some((tp) => isExactTimePoint(timeParts.totalSeconds, tp));
                    }
                    break;
                case "weekday":
                    if (cond.weekdays?.length > 0)
                        result = cond.weekdays.includes(timeParts.weekday);
                    break;
                case "sensor_value": {
                ...
                case "status": {
                    ...
                    if (sensor)
                        result = String(sensor.value) === String(cond.expected_status);
                    break;
                }
            }
            results.push(result);
...
async function executeWithRetry(entityId, cmdValue) {
...
async function executeHAService(entityId, cmdValue) {
...
async function syncAutomationsFromCloud() {
    ...
    try {
        ...
        if (!data.success || !Array.isArray(data.automations))
            return;
        const upsert = db.prepare(`INSERT OR REPLACE INTO automations_local (id, data, updated_at, last_executed_at)
       VALUES (?, ?, ?, COALESCE((SELECT last_executed_at FROM automations_local WHERE id = ?), NULL))`);
        const syncTransaction = db.transaction((automations) => {
            for (const auto of automations) {
                const ruleData = {
                    ...auto,
        ...
        if (isFullSync) {
            const cloudIds = new Set(data.automations.map((a) => a.id));
...
async function pushExecutionLogs() {
    // Always attempt – connectivity is tracked by heartbeat/sync results
    const unsyncedLogs = db.prepare(`SELECT * FROM automation_exec_log WHERE synced = 0 ORDER BY id LIMIT 100`).all();
...
async function flushBuffer() {
    ...
    const rows = fetchBatch.all(FLUSH_BATCH_SIZE);
    if (rows.length === 0)
        return;
 ...
 */
async function pushDeviceSnapshot() {
    if (!isCloudReachable && !cloudWsConnected)
        return;
    if (latestHAStates.length === 0)
        return;
    ...
    console.log(`[snapshot] inventory analysis: ha_states=${latestHAStates.length} ignored=${ignoredCount} ` +
...
function loadGatewayAssignmentFromCache() {
    return db.prepare(`
...
function saveGatewayAssignmentToCache(assignment, assignmentStatus) {
    db.prepare(`
  ...
  `).run('primary', assignment.device_id || null, assignment.tenant_id || null, assignment.tenant_name || null, assignment.location_id || null, assignment.location_name || null, assignment.location_integration_id || null, assignmentStatus);
}
...
async function connectCloudWebSocket() {
    if (!WebSocketClient) {
 ...
 */
async function handleCloudCommand(cmdType, payload) {
    switch (cmdType) {
        case "execute_actuator": {
            ...
            if (!entityId)
                throw new Error("entity_id missing");
        ...
        case "discover_devices": {
            ...
            const methods = Array.isArray(payload.methods) ? payload.methods : ["mdns", "mqtt"];
            const found = await runDeviceDiscovery(methods, payload.modbus);
        ...
        case "provision_entity": {
            ...
            const result = await provisionEntity(payload);
            await reportProvisionStatus(String(payload.entity_id || ""), result);
            return result;
        }
        case "deprovision_entity": {
            const result = await deprovisionEntity(payload);
            return result;
        }
        case "pull_image": {
            ...
            if (!jobId || !imageRef)
                throw new Error("job_id/image_ref missing");
        ...
        case "update_wallbox": {
            ...
            if (!instanceId)
                throw new Error("instance_id missing");
            const data = await fetchWallboxInstanceAndTemplate(instanceId);
            if (!data)
                throw new Error(`wallbox instance ${instanceId} not found`);
...
// -------------------------------------------------------------------------
function reportUpdateProgress(jobId, status, extra = {}) {
    if (!cloudWs || !cloudWsConnected)
        return;
    safeWsSend(cloudWs, {
...
async function runUpdateJob(jobId, imageRef, targetVersion) {
    ...
    try {
        ...
        console.warn("[update] Supervisor update failed:", err.message);
        reportUpdateProgress(jobId, "failed", { error: err.message });
    }
}
function persistPendingUpdateJob(jobId, targetVersion) {
    try {
      ...
      created_at TEXT NOT NULL DEFAULT (datetime('now'))
        ...
        console.warn("[update] persist failed:", err.message);
    }
}
function reportPostBootUpdateResult() {
    try {
        db.prepare(`CREATE TABLE IF NOT EXISTS pending_update_jobs (
      job_id TEXT PRIMARY KEY,
      target_version TEXT NOT NULL,
      created_at TEXT NOT NULL DEFAULT (datetime('now'))
        ...
        const rows = db.prepare(`SELECT job_id, target_version FROM pending_update_jobs`).all();
        ...
        console.warn("[update] post-boot report failed:", err.message);
    }
}
async function runDeviceDiscovery(methods, modbus) {
    const results = [];
    ...
    if (methods.includes("mdns") || methods.includes("mqtt")) {
        try {
            ...
            console.warn("[discover] HA state fetch failed:", err.message);
        }
    }
...
async function fetchHAStatesRaw() {
    ...
    const cache = globalThis.__lastHaStates;
    if (Array.isArray(cache))
        return cache;
    return [];
}
function guessIntegrationFromEntity(entityId, state) {
    const eid = entityId.toLowerCase();
    const platform = String(state?.attributes?.platform || "").toLowerCase();
    if (platform === "shelly" || eid.includes("shelly"))
        return "shelly";
    if (platform === "mqtt" || platform === "tasmota")
        return "tasmota";
    if (platform === "esphome")
        return "esphome";
    if (platform === "modbus")
...
function isEntityMapped(entityId) {
    try {
        return meterMappings.some((m) => m.ha_entity_id === entityId || m.sensor_uuid === entityId);
    }
    catch {
        return false;
    }
}
async function probeModbusUnit(host, port, _unit) {
...
async function pushDiscoveriesToCloud(items) {
    if (items.length === 0)
        return;
    try {
        safeWsSend(cloudWs, {
        ...
        console.warn("[discover] push failed:", err.message);
    }
}
...
async function deprovisionEntity(payload) {
    try {
        if (payload.integration_type === "modbus_tcp") {
...
function saveLocalIntegrationConfig(entityId, config) {
    try {
        db.prepare(`
    ...
    `).run(entityId, JSON.stringify(config), Date.now());
        ...
        console.warn("[provision] local persist failed:", err.message);
    }
}
function removeLocalIntegrationConfig(entityId) {
    try {
        db.prepare(`DELETE FROM gateway_local_integrations WHERE entity_id = ?`).run(entityId);
    }
    catch { /* table may not exist yet */ }
}
async function reportProvisionStatus(entityId, result) {
    if (!entityId)
        return;
    try {
        safeWsSend(cloudWs, {
...
async function executePendingCommand(command, params) {
...
const UI_DIR = path_1.default.join(__dirname, "ui");
function serveStaticFile(filePath, res) {
    ...
    const fullPath = path_1.default.join(UI_DIR, filePath);
    const safePath = path_1.default.resolve(fullPath);
    if (!safePath.startsWith(path_1.default.resolve(UI_DIR))) {
        res.writeHead(403);
        res.end("Forbidden");
        return;
    }
    try {
        const content = fs_1.default.readFileSync(safePath);
        const ext = path_1.default.extname(safePath);
    ...
    const server = http_1.default.createServer(async (req, res) => {
        ...
        if (pathname === "/api/wallboxes") {
            ...
            try {
                ...
                console.warn("[api] /api/wallboxes failed:", e.message);
            }
            res.writeHead(200, { "Content-Type": "application/json" });
        ...
        if (pathname === "/api/logs") {
            ...
            const logs = db.prepare(`SELECT automation_id, tenant_id, status, error_message, duration_ms, trigger_type, created_at
         FROM automation_exec_log ORDER BY id DESC LIMIT ?`).all(Math.min(limit, 200));
            const automationRows = db.prepare(`SELECT id, data FROM automations_local`).all();
...
async function main() {
    ...
    if (cachedAssignment.location_id || cachedAssignment.location_name || cachedAssignment.tenant_name) {
        ...
        currentAssignmentStatus = cachedAssignment.assignment_status || (cachedAssignment.location_name ? "assigned" : "unknown");
    ...
    void (async () => {
        try {
            ...
            console.warn("[boot] cloud check failed:", e.message);
        }
        try {
            await fetchHAVersion();
        }
        catch (e) {
            console.warn("[boot] HA version failed:", e.message);
        }
        try {
            await fetchMeterMappings();
        }
        catch (e) {
            console.warn("[boot] meter mappings failed:", e.message);
        }
        try {
            await syncAutomationsFromCloud();
        }
        catch (e) {
            console.warn("[boot] automation sync failed:", e.message);
        }

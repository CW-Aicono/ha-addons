...
function readOptions() {
    try {
        return JSON.parse(fs_1.default.readFileSync(OPTIONS_PATH, "utf-8"));
    }
    catch {
        return {};
    }
...
function writeOptions(next) {
    fs_1.default.writeFileSync(OPTIONS_PATH, JSON.stringify(next, null, 2));
}
/** True wenn Wizard laufen muss. */
function needsSetup(opts = readOptions()) {
    return !opts.gateway_username || !opts.gateway_password;
}
/** Erste nicht-loopback IPv4-Adresse für QR-Code/Anzeige. */
function localIPv4() {
    for (const list of Object.values(os_1.default.networkInterfaces())) {
        for (const i of list || []) {
...
<service-group>
  ...
  <service>
    ...
    try {
        fs_1.default.mkdirSync("/etc/avahi/services", { recursive: true });
        fs_1.default.writeFileSync("/etc/avahi/services/aicono.service", service);
        // Hostname setzen – Avahi nimmt /etc/hostname als Basis
        fs_1.default.writeFileSync("/etc/hostname", "aicono\n");
        (0, child_process_1.execSync)("hostnamectl set-hostname aicono || true", { stdio: "ignore" });
        (0, child_process_1.execSync)("systemctl reload avahi-daemon || service avahi-daemon reload || true", { stdio: "ignore" });
    }
    catch (e) {
        console.warn("[setup] mDNS-Konfiguration fehlgeschlagen:", e.message);
    }
}
const HTML = (ip) => `<!doctype html>
<html lang="de"><head>
<meta charset="utf-8"/>
...
async function pairWithCloud(token, mac, deviceName, cloudUrl) {
    ...
    for (const list of Object.values(os_1.default.networkInterfaces())) {
        for (const i of list || []) {
            if (!i.internal && i.mac && i.mac !== "00:00:00:00:00:00")
                return i.mac.toLowerCase();
        }
    }
    return "";
}
function runSetupWizard(port = 8099) {
    announceMdns();
    const ip = localIPv4();
                ...
                req.on("end", async () => {
                    try {
                        ...
                        if (!token || typeof token !== "string")
                            throw new Error("Kein Token übergeben");
                        const opts = readOptions();
                        const cloud = opts.cloud_url || DEFAULT_CLOUD;
                        const deviceName = opts.device_name || `aicono-${mac.slice(-5).replace(":", "")}`;
